<x-app-layout>
    <!-- Title -->
    <x-title text="Menu Presensi Siswa"/>

    <!-- Page content -->
    <x-app-card>
        <!-- Card header -->
        <x-slot name="header">
            Tabel Data Presensi {{ $student->name }}
        </x-slot>
            ...
            $(function () {
                ...
                let table = $('#dataTable').DataTable({
                    ...
                    buttons: [{
                        ...
                        {
                            extend: 'csv',
                            title: 'Data Presensi  {{ $student->name }}',
                            exportOptions: {
                                columns: ':visible'
                            }
                        },
                    ...
                    ajax: {
                        'url': '{{ route('admin.students.attendances.index', $student) }}',
                        'data': function (d) {
                            d.days = $('#days').val();
                        }
                    },
                    columns: [{
                        ...
                        {
                            data: 'schedule.classroom.subject.teacher.name',
                            name: 'schedule.classroom.subject.teacher.name'
                        },
                        {
                            data: 'latitude',
                            name: 'latitude'
                    ...
                    columnDefs : [{
                        render : function (data, type, row) {
                            return data + ' - ' + row['schedule']['classroom']['subject']['name'];
                        }, 'targets' : 2 },
                        {
                        render : function (data, type, row) {
